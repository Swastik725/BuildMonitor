...
original FastAPI plan would waste days of the 10-day window for no benefit — the architecture
...
Decision: No Redis / caching layer in V1
Status: Accepted
...
Tradeoffs: Will need to be added back once there's real concurrent load — deferred to post-V1,
...
Status: Accepted
Reason: There is no genuinely slow/async workload in the V1 feature set — deployments, metrics,
...
Decision: Deployments, metrics, and health checks are simulated in V1, not real
Status: Accepted
Reason: Building a real CI/build/deploy engine and real infrastructure to monitor is a multi-week
project on its own — far outside a 10-day MVP window, and not actually the point (BuildMonitor is
...
Decision: No GitHub OAuth or webhooks in V1; manual read-only repo connection instead
Status: Accepted
...
post-V1 (see `GITHUB_INTEGRATION.md`).
